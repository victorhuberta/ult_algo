...
macro_rules! include_sequence_search {
    () => {
        ...
        use ult_algo::sequence::search::{binary, ternary, SearchTarget};
    };
}
...
mod ternary_tests {
    ...
    fn finds_max_and_receives_mod_function() {
        ...
        assert_eq!(
            ternary(search_target, |x| x % 5.0, 25.4, 30.1, 0.05),
            29.990194395991274
        );
    }
    ...
    fn finds_max_and_receives_power_function() {
        let search_target = SearchTarget::Maximum;
        assert_eq!(
            ternary(search_target, |x| x.powf(x), 25.4, 30.1, 0.00001),
            30.099996368748634
        );
    }
    ...
    fn finds_max_and_receives_smaller_right() {
        let search_target = SearchTarget::Maximum;
        assert_eq!(
            ternary(search_target, |x| x % 5.0, 30.1, 25.4, 0.05),
            29.990194395991274
        );
    }
    ...
    fn use_ternary_max_macro() {
        assert_eq!(
            ternary_max!(|x| x % 5.0, 25.4, 30.1, 0.05),
            29.990194395991274
        );
    }
    ...
    fn use_ternary_min_macro() {
        assert_eq!(
            ternary_min!(|x| x.powf(x), 25.4, 30.1, 0.00001),
            25.400003631251366
        );
    }
}
...
macro_rules! binary_successor {
    ($sequence:expr, $val:expr) => {{
        if $sequence.len() == 0 {
            ...
            if result.index == None && result.rank == 0 {
                // target is out of range and smaller
                Some(0)
            } else {
                let successor_idx = if result.index == None {
                    result.rank
                } else {
                    result.rank + 1
                };
...
where
    T: Copy + PartialOrd + PartialEq + ops::Sub<Output = T>,
{
    if sequence.len() == 0 {
        return None;
    }
        ...
        + ops::Div<Output = T>,
{
    ...
    while sequence[high] != sequence[low] && (*val >= sequence[low] && *val <= sequence[high]) {
